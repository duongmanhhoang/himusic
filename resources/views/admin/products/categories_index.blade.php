                ...
                <table class="m-datatable" id="html_table" width="100%">
                    ...
                    @foreach($categories as $category)
                        <tr>
                            ...
                            <td>
                                <?php
                                $cate = \App\Model\Category::where('type', 'product')->where('id', $category->parent_id)->first();
                                ?>
                                @if(isset($cate))
                                    {{$cate->name}}
                                @else
                            ...
                            <td>
                                ...
                                <a href="javascript:;" linkurl="{{route('admin.products.categories.delete' , $category->id)}}"
    ...
    <script>
        $(document).ready(function () {
            $(".btn-remove").click(function (e) {
                ...
                swal({
                    title: "Bạn có chắc muốn danh mục sản phẩm này?",
                    text: "",
                    type: "warning",
                    showCancelButton: !0,
        ...
        @if(session('store'))
        swal('Thêm danh mục sản phẩm thành công', '', 'success');
        @endif
        @if(session('delete'))
        swal('Xóa danh mục sản phẩm thành công', '', 'success');
        @endif
    </script>
@endsection